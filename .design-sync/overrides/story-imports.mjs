...
export function storyImportPlugins({ PKG, GLOBAL, extraEntries = [], exported, cfg, pkgDir }) {
  ...
  // FORK (balena ui-shared-components): the DS re-exports whole vendor
  // packages as namespaces on its global (Material, MaterialLab, ReactQuery,
  // …). Stories import those packages directly (`import { Button } from
  // '@mui/material'`); bundling them fresh into each preview creates a SECOND
  // MUI whose theme context differs from the provider's bundle instance
  // (story-local elements render unthemed) and adds ~300KB per preview.
  // Adding '@mui/material' to extraEntries instead would star-merge it onto
  // the global and silently drop names that collide (react-router-dom's Link
  // vs MUI's Link — SettingsCard imports the MUI one). Mapping the specifier
  // onto the existing namespace export keeps ONE instance and zero collisions.
  const NS_MAP = {
    '@mui/material': 'Material',
    '@mui/lab': 'MaterialLab',
    '@mui/x-data-grid': 'MaterialDataGrid',
    '@mui/x-date-pickers': 'XDatePickers',
    '@tanstack/react-query': 'ReactQuery',
  };
  const nsRx = new RegExp(`^(?:${Object.keys(NS_MAP).map(escRx).join('|')})$`);

  const dsShim = {
    name: 'ds-global',
    setup(b) {
      const entryNames = new Set([PKG, ...extraEntries]);
      b.onResolve({ filter: nsRx }, (a) => {
        if (matches(a.path, force.bundle)) return null; // explicit bundle wins
        return { path: `ds-ns:${NS_MAP[a.path]}`, namespace: 'ds-ns-shim' };
      });
      b.onLoad({ filter: /.*/, namespace: 'ds-ns-shim' }, (a) => ({
        contents: `module.exports=window.${GLOBAL}.${a.path.slice('ds-ns:'.length)};`,
        loader: 'js',
      }));
