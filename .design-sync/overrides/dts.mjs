...
// FORK (balena ui-shared-components): RJST ("React JSON Schema Table") is a
// real, storied component whose name is all-caps, so the stock all-caps
// rejection (`/^[A-Z][A-Z0-9_]+$/`) misclassifies it as an enum/constant and
// drops it from the build. Exempt it by name; every other rule is stock.
const ALL_CAPS_COMPONENT_EXEMPT = new Set(['RJST']);
export const isComponentName = (n) => !n.endsWith('Props')
  && (ALL_CAPS_COMPONENT_EXEMPT.has(n) || !/^[A-Z][A-Z0-9_]+$/.test(n))
