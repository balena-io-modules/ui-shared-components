...
const getDefaultValue = (data?: FormData, propertySchema?: JSONSchema) => {
...
export const FiltersDialog = ({
	...
	const handleChange = React.useCallback(
		({ formData: fData }: IChangeEvent<FormData[]>) => {
			...
			setFormData(normalizeFormData(schema, newFormData));
		},
				...
				items: formData.map((fd, index) => {
					...
					if (propertySchema?.type === 'object') {
						...
						uiSchema.items.value = Object.fromEntries(
							Object.entries(propertySchema.properties).map(([key, value]) => [
								key,
								{
									'ui:title': '',
									'ui:placeholder': (value as JSONSchema).title + '*',
								},
							]),
						);
					}
