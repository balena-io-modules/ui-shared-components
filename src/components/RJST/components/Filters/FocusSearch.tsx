...
const Focus = styled(Box)(({ theme }) => ({
	...
	border: `solid 1px ${theme.vars.palette['b-border-subtle']}`,
	maxHeight: '200px',
	position: 'absolute',
	width: '100%',
	zIndex: 1,
	borderRadius: `0 0 ${theme.vars.shape.sm} ${theme.vars.shape.sm}`,
	overflow: 'hidden',
...
const FocusItem = styled(Box)<{ hasGetBaseUrl: boolean }>(
	({ hasGetBaseUrl, theme }) => ({
		...
		'&:hover': {
			backgroundColor: theme.vars.palette['b-bg'], // This is the background color MUI Select uses for entities on hover.
		},
	}),
);
