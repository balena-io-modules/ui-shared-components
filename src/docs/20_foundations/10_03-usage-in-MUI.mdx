...
* `color.bg.accent` -> `var(--mui-palette-bg-accent)`
...
Contrary to the other variables, color tokens are separated by dashes instead of dots. The reason is that MUI infers 
the variable names based on the object passed in the theme (hence the dots), but we use implicit defaults for our
token nomenclature resulting in potential clashes like `color.bg` and `color.bg.accent`. 

Examples:

* `color.bg.accent` -> `theme.vars.palette['bg-accent']` 
* `color.text.subtle` -> `theme.vars.palette['text-subtle']` 
* `color.bg.warning.strong ` -> `theme.vars.palette['bg-warning-strong']` 
...
```tsx
<Box sx={{ backgroundColor: 'bg-accent-strong', color: 'text-inverse', borderRadius: 'lg', p: 2 }}>
